(function($, ko){
	...
	(function(ns) {
		//VM for the entire Talent Tree page，这里的vm应该是ViewModel的意思
		var TalentTree = ns.TalentTree = function(_e){
			...
			//Portrait stuff
			self.portrait = ko.observable(Math.ceil(Math.random() * numPortraits));//随机取得一个头像
		...
		var Skill = ns.Skill = function(_e, allSkills){
			...
			self.dependencies = ko.observableArray([]);//我依赖的
			self.dependents = ko.observableArray([]);//依赖我的
			self.stats = e.stats || [];
